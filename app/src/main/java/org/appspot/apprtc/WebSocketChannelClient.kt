 ...
 */
class WebSocketChannelClient(private val handler: Handler, private val events: WebSocketChannelEvents) {
...
//    private val closeEventLock = Any()
    ...
    private val closeEventCond = closeEventLock.newCondition()
    private var closeEvent = false

    // WebSocket send queue. Messages are added to the queue when WebSocket
    // client is not registered and are consumed in register() call.
    private val wsSendQueue: MutableList<String> = ArrayList()
...
//                            closeEventLock.wait(CLOSE_TIMEOUT.toLong())
                            closeEventCond.await(CLOSE_TIMEOUT.toLong(), TimeUnit.MILLISECONDS)
                            break
