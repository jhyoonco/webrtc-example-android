...
class CaptureQualityController(
        private val captureFormatText: TextView?, private val callEvents: OnCallEvents?) : OnSeekBarChangeListener {
    private val formats = Arrays.asList(CaptureFormat(1280, 720, 0, 30000), CaptureFormat(960, 540, 0, 30000),
    ...
    override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
        ...
        for (format in formats) {
            maxCaptureBandwidth = Math.max(maxCaptureBandwidth, format.width.toLong() * format.height * format.framerate.max)
        }
